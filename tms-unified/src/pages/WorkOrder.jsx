...
export default function WorkOrder() {
  ...
  const tabs = [
    { id: 'subdivide', name: '소분 작업지시서', icon: <Grid size={18} />, path: '/task/subdivide' },
    { id: 'schedule', name: '작업 일정 달력', icon: <Calendar size={18} />, path: '/task/schedule' },
  ];
    ...
    <div className="w-full h-full p-4 sm:p-6 animate-fade-in flex flex-col">
      <div className="bg-white/70 backdrop-blur-md rounded-[2rem] shadow-sm border border-white/80 p-4 sm:p-6 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm font-bold text-slate-500 mt-1">소분 지시서(매트릭스 뷰) 출력 및 작업 일정표</p>
        </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-black transition-all whitespace-nowrap ${
                  isActive ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50'
                }`}
              >
                {tab.icon} {tab.name}
              </button>
